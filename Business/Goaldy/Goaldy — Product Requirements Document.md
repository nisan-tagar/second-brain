...
A pre-public-launch security assessment found **no brute-force protection whatsoever** on the login endpoint — unlimited password attempts, no lockout, no rate limiting. Shipped: per-identity rate limiting and progressive lockout on failed logins (keyed on the submitted email, so an unknown address is throttled identically to a real one — no enumeration signal), session-ID rotation on every login, a 7-day idle session timeout in addition to the 30-day absolute expiry, timing-safe comparison of the API bearer token's hash, and app-wide security response headers (CSP, `X-Frame-Options`, `Strict-Transport-Security`, `X-Content-Type-Options`, `Referrer-Policy` — `apps/web/lib/server/security-headers.ts`). The README now documents, plainly, that a reverse proxy terminating TLS is required (not optional) for any deployment reachable from outside the operator's own network, with a Caddy example and the `docker-compose.yml` port-binding change to pair with it. See `docs/superpowers/specs/2026-09-04-security-hardening-household-auth.md` and `docs/superpowers/plans/2026-09-04-security-hardening-household-auth.md` in the app repo.
...
- Goaldy ships with **no users and no default password of any kind.** The app is unusable — not "usable with a known credential" — until someone completes setup.
- First boot presents a **setup wizard**, gated by a **one-time setup token printed to the container logs** (the same reveal-once pattern already used for the API bearer token). The wizard collects, in one step: a **workspace label** (a free-text name for the deployment — "Tagar Family," a company name, anything — shown thereafter on the login screen and in the app's topbar) and the first real user's name, email, and a strength-validated password.
- **Why the token matters:** without it, an empty-users-table design is itself a race — on a public VPS (reachable by definition), whichever HTTP request reaches the freshly-booted instance first could claim the account. Requiring a token only the operator can read from their own container logs closes that race outright, regardless of how long the instance sits exposed before they finish setup.
