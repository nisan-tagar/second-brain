...
3. **Every top-level category**, with **change and trend in separate columns**.
...
**Answered and shipped.** The hint's home is the moment the amount is being chosen — the set-a-budget form — where the tag's typical figure, its range and how steady it is belong on screen at once, rather than a single average bolted to a history row. Setting a budget now opens with a line like *"Based on the last 12 periods: typically ₪2,340 — range ₪1,900 to ₪4,100. Spending here is steady."* and a one-click **Use ₪2,400**.

**The suggestion is the median, never the mean.** One December sets a mean and does not move a median, and a budget talked upward by a single anomaly is one the user blows through every ordinary month — which teaches them the number is noise. Where spending is genuinely erratic the suggestion moves to the 75th percentile instead, so the headroom is sized by the category's own history rather than by an invented margin, and the line says that is what happened. Suggestions round **up**: a budget rounded down sits below what the history says the category costs, which is the one direction that makes it wrong the moment it is accepted.

**The in-progress period is excluded.** A month that is half elapsed is half spent by construction, and letting it into the typical figure would talk the suggestion down toward an amount the user has never actually achieved. Below three completed periods there is no suggestion at all — just a line saying there is not enough history yet, never a figure drawn from an anecdote.
...
### F7.8 Pace — the time axis the progress bar never had

A progress bar shows how much of a budget is spent. It cannot show how much of the *period* is gone, which means **60% spent on day 5 and 60% spent on day 25 render identically** — opposite situations, the same picture. Every budget row now carries a marker at the period's elapsed position, so the gap between the fill and the marker is the answer at a glance, plus one line of plain language beneath it.

**One line, not three.** In priority order: a run of consecutive overspends (a habit, not a month), a budget the user routinely exceeds (a configuration problem, not a spending one), then the live pace — *on course for ₪2,800*. Showing all three turns a list into a report; the ranked review of everything at once is a separate surface the user asks for (F19 §9a C4), not something a list becomes by accident.

**The marker is not red.** Red in this product means money went out (§1.4). A projection that exceeds the budget has not happened yet, and colouring it as an overspend would assert a fact the data does not support. The marker's position carries the meaning and the sentence states it.

**Income targets get the projection and no verdict.** The underlying figures count occurrences *above* the budgeted amount — which reads as "you went over" on an expense budget and "you beat it" on an income target. One arithmetic cannot serve both honestly, so an income row is told what it is on course for and is not graded.

**Nothing is shown that cannot be supported.** The engine withholds a pace below 10% elapsed (one early grocery run would report being 600% hot), withholds the history figures below three complete periods, and withholds all of them for a rollover budget, whose allowance is a running figure rather than a fixed one. In each case the row simply shows no line — never a zero, and never a guess.
...
In order: the tag detail view (**done** — and this is where the duplicated average got deleted), the **Budgets page (done — see F7.8)**, the **account detail view (done)**, the **set-a-budget flow (done — F7.4)**, the dashboard's period-over-period deltas (F6.1, still pending `compare`), and finally Reports (F10) as a comparison table. Plus one AI tool, `get_metrics`, shared by both F18 tiers — carrying the same prohibitions in its description that the UI enforces in pixels, since a tool description is the only instruction an external agent ever receives.

**What the surfaces show, and what they refuse to.** A metric figure appears with the sample size behind it, and where the sample cannot support one the app shows the figure's name, a dash, and what would fix it — never a hidden row and never a number. A direction ("trending up") is only stated when the fit supports one; below that the app says the spending is flat, whichever way the line happens to lean. An "unusual month" is only called out when it genuinely is one — a normal month gets no comment at all, because an attention list that mentions everything gets read as wallpaper. Account figures stay in the account's own currency and are simply not shown if the app would have to convert them to display them, since a wrong currency label on a money figure is worse than an absent figure.
