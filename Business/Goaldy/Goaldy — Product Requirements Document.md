...
> **Blocked on the monetary-representation fix (TDD §15, BUG-002).** Every monetary column in the ledger is currently a binary float, and this feature does precisely the arithmetic that exposes that: summing thousands of rows, multiplying each by a split weight, then measuring how much they vary. Building a statistics engine on a numeric representation already known to be wrong means building it twice, so the representation is fixed first. Nothing about F19's design changes as a result — only its start date.
