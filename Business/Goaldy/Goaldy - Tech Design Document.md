...
|2.15|2026-09-14|**The threshold table had a fourth copy, and v2.13 above described it as a reader.** That entry claimed "the `get_metrics` tool description carries the same bands as the table's third consumer" — true only because the numbers had been typed out by hand into an English sentence. A tool description is a static string, so it cannot CALL a band function; it stated *"cv: under 0.15 'very predictable'"* and *"do not assert a trend when r2 is below 0.3"* in prose. `metric-band-guard` scans for a banded metric compared against a bare number and is therefore structurally blind to it: retuning `cvBand` would have left the model quoting the retired boundary indefinitely, and the mismatch would have surfaced to the user as the chat and the screen describing the same tag differently — §9b's exact failure, reappearing in the one consumer the enforcement could not reach. **Band edges are now data** (`CV_EDGES`, `R2_EDGES`, …), every band function is one shared `pickBand`, and `agentBandGuidance()` GENERATES the tool's prose from those arrays. A guard asserts the description states every edge the table defines and no number it does not. The general rule, which the next consumer will need: **a consumer that cannot call the band functions must generate its text from the edges** — a threshold inside a sentence outlives every retuning.|
|2.16|2026-09-14|**`compare` shipped, and the engine is now feature-complete against its design.** The parameter was reserved — accepting only `none`, 400ing on everything else — precisely so widening it would not be a breaking change, and this is that widening: `previous-period` and `previous-year` both answer, and a caller that never passed it is unaffected. **The comparison is the same function recursed once over a shifted window**, so no second implementation exists to drift from the first. Two things are pinned rather than inherited by that recursion, both correctness rather than taste: the **grain** (left to resolve itself, the prior window could infer a different period and bucket differently, and a difference between series bucketed differently is not a difference) and **`anchor: 'horizon'`** (`first-activity` moves a window's start forward to the member's earliest transaction, which applied to the prior window would silently shorten the very window whose length is the basis of the comparison). **Three refusals travel with every delta**, in one module so they cannot be applied to some metrics and not others: mixed currency withholds everything (D14), either window at sufficiency `none` withholds everything, and a percentage is refused on a zero base or across a sign change — though the DIFFERENCE survives both, because "you spent 1,200/mo, up 1,200 from nothing" is a fact while "up ∞%" is not. A withheld delta propagates the metric's original reason rather than restating everything as `insufficient-buckets`. `previous-year` shifts twelve MONTHS, not 365 days: a comparison that drifted each leap cycle would stop comparing like with like, and 29 Feb clamps to 28 Feb rather than overflowing into March. Its consumer is the new **Trends** dashboard view (PRD F6.5), which replaced the `/reports` placeholder. Trends is **Tier 3 and mounted only when its tab is selected** — D26's rule that nothing whole-tree runs on a page load is what keeps the Dashboard's load cost where it is, and rendering all three dashboard bodies eagerly would quietly undo it. **One gap found by building the consumer:** the engine takes no account filter at any dimension, so the dashboard's account selector cannot scope Trends. Spreading `accountIds` into the query was silently dropped — TypeScript's excess-property check does not fire through a spread — which would have left a filter visibly set and secretly ignored. The view states the limit; scoping the engine by account is follow-on work.|
